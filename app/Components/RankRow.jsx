...
const RankRow = (props) => {
  ...
  // } else {
    return (
      <div className='rankingsRow'>
        <div>{props.rank+1}</div>
        <div className='userNameDiv'>{props.ranking.username}<button className='inviteBtn' onClick={() => props.createMatch({host_id: props.loginID, guest_id: props.ranking.id})} className='matchBtn'>Invite!</button></div>
        <div>{props.ranking.points}</div>
        <div>{props.ranking.wins}</div>
        <div>{props.ranking.losses}</div>
        {/* <div>{props.ranking.points}</div> */}
